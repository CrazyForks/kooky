...
/// invoke it without a password. From then on kooky toggles lid sleep
...
enum ClosedLidSleep {
    static let helperPath = "/Library/PrivilegedHelperTools/kooky-sleepctl"
    /// No dot in the filename — sudoers.d silently ignores files containing one.
    static let sudoersPath = "/etc/sudoers.d/kooky-sleepctl"
    ...
    cat > /Library/PrivilegedHelperTools/kooky-sleepctl <<'HELPER'
    \(helperScript)
    HELPER
    chown root:wheel /Library/PrivilegedHelperTools/kooky-sleepctl
    chmod 755 /Library/PrivilegedHelperTools/kooky-sleepctl
    t=$(mktemp)
    printf '%s ALL=(root) NOPASSWD: /Library/PrivilegedHelperTools/kooky-sleepctl on, /Library/PrivilegedHelperTools/kooky-sleepctl off\\n' "$u" > "$t"
    /usr/sbin/visudo -cf "$t"
    cp "$t" /etc/sudoers.d/kooky-sleepctl
    chown root:wheel /etc/sudoers.d/kooky-sleepctl
    chmod 440 /etc/sudoers.d/kooky-sleepctl
    rm -f "$t"
    """
    ...
    static func install(completion: @escaping @MainActor (Bool) -> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let scriptURL = dir.appendingPathComponent("kooky-lid-install-\(UUID().uuidString.prefix(8)).sh")
            do {
    ...
    /// Marker file recording "kooky engaged SleepDisabled". Written before
    /// every engage, removed after a confirmed release. Lets launch
    /// reconciliation distinguish kooky's own crash straggler (safe to
    /// clear) from a SleepDisabled the user or another tool set on purpose
    /// (absorb, never clear).
    private static var ownershipMarkerURL: URL {
        KookyShellIntegration.kookyAppSupport("lid-engaged", isDirectory: false)
    }
