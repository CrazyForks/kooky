...
final class SleepGuard {
    ...
    var isKeepingAwake: Bool { assertionID != nil }
    ...
    var awakeMode: @MainActor () -> AwakeMode = {
        KookySettingsModel.shared.awakeMode
    }
    ...
    var setMode: @MainActor (AwakeMode) -> Void = {
        KookySettingsModel.shared.applyAwakeMode($0, runInstall: false)
    }
    ...
    var onLidHelperFailure: @MainActor () -> Void = {
        if KookySettingsModel.shared.awakeMode == .always {
            KookySettingsModel.shared.applyAwakeMode(.auto, runInstall: false)
        }
    }
    ...
    /// another tool, and kooky's light/Settings must not show stale state.
    /// No change notification exists for it, so: poll every 30s, plus an
    /// immediate check whenever kooky becomes the active app (the moment
    /// right after someone flipped it in a terminal). Only meaningful once
    /// the helper exists (without it kooky never engages the lid layer and
    ...
    private func startSystemStateWatch() {
        ...
        Task { @MainActor [weak self] in
            while let self, self.observing {
                ...
                // reconciles external changes the moment kooky is fronted.
    ...
    /// Absorb an out-of-band SleepDisabled change so kooky reflects reality.
    ...
    /// External ON surfaces as the Always notch (switching away in kooky
    /// then releases it).
    func reconcileExternalLidState(systemDisabled: Bool) {
        ...
        lidReleaseBlocked = false
        if systemDisabled {
            markOwnership()   // kooky manages it from here
    ...
    /// - marker present → kooky set it. Crash straggler: clear it — unless
    ...
    private func reconcileAtLaunch() async {
        ...
        if system {
            if ClosedLidSleep.ownsLidState && awakeMode() != .always {
                ...
                // Not ours → surface as Always; kooky takes ownership and
                // dialing away in kooky will release it.
